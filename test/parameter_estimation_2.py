...
bounds = [(-10.0, 10.0), (-10.0, 10.0), (-10.0, 10.0)]  # Adjust these bounds as necessary
...
def objective(obj_params, plot=False, smooth=False):
    ...
    try:
        for (measure, m) in zip(measurements, masses):
            f_estimateds = []
            if not smooth:
                for (f_measured, a_motor, v, i, t) in zip(measure['force'], measure['am'], measure['vm'], measure['Im'], measure['time']):
                    """
                    force = dtheta_dot_dt*params[0] - theta_dot * params[1] + i * params[2]
                    ...
                    total_cost += abs(float(f_measured) - f_estimated)
            if plot:
                plt.figure()
